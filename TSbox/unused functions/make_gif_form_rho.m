function make_gif_form_rho(Frho_all_stat,Frho_all_1st,sigma0,n_stat,fullname,e1,ocean,Frho_all_mean,sal,temp,tframe,evapo,prec,hflux,radflux)
...
%     (errorbar(x1,y1,e1,'color',mblu,'linewidth',0.5));
    ...
    if strcmp(sal(1,:),'aqOI   ')
        legend (['Aquarius/OSTIA for n = ',num2str(i)],'Aquarius/OSTIA','WOA09','Argo','Mean of 50 MC','Error','Location','southeast')
    else
        legend (['SMOS/OSTIA for n = ',num2str(i)],'SMOS/OSTIA','WOA09','Argo','Mean of 50 MC','Error','Location','southeast')
    end
...
original_dir = pwd;
mkdir([original_dir,'\gifs\','annual_form_rho/',tframe,'/',sal(1,1:end),'/',temp(1,1:end-3),'/',hflux,'/',radflux,'/',evapo,'/',prec,'/',ocean(1:2),'/mc',num2str(n_stat)])
cd ([original_dir,'\gifs\','annual_form_rho/',tframe,'/',sal(1,1:end),'/',temp(1,1:end-3),'/',hflux,'/',radflux,'/',evapo,'/',prec,'/',ocean(1:2),'/mc',num2str(n_stat)])
