...
Frho_error = nan(n_stat,length(latitude),length(longitude),81,75);
%% 1.1 load data
[SSS lat lon] = load_all_v2(sal,tframe,'SSS');
...
for n = 1:n_stat
    ...
    rrrr = nan(size(orho,1),size(orho,2),length(S),length(T),12);
        ...
        for m = 1:12
            ...
            for t = 2:length(S)
               waitbar(t/length(S),f,['Computing...'])
...
Frho_error = permute(Frho_error,[2,3,4,5,1]);
