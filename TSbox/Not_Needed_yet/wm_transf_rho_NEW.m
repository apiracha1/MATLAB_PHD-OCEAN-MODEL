...
if isa(sal_name,'char')
    sal = char(sal_name,'argoNRT'); 
else
    sal = char(sal_name{2},'argoNRT'); 
end
temp = char(temp_name,...
    'argoNRT'); 
...
for years = 1:length(time_frame)    
    ...
    %% 0. parameters
                ...
                for sss = 1:2 % loop over products
                    %% 1.1 load data
                if isa(sal_name,'char') || sss > 1
                    [SSS lat lon] = load_all_v2(strcat(sal(sss,:)),tframe,'SSS');
                else
                    [SSS,lat,lon,longsss] = seperate_user_dataset (sal_name,tframe);
                end
...
wm_form_rho_NEW(sal,temp,time_frame,...
        ocean,prec,n_stat,month,evap_name,seasons,sal_name) 
close(msg)
end
